...
<strong>Note</strong>: Enabling this feature is something of a judgment call. If enabled, passwords which don’t match the specified criteria will be rejected by MySQL with an error. It is safe to leave validation disabled, but you should always use strong, unique passwords for database credentials.
...
to confirm that PHP is running type 

```bash
php -v
```
![PHP](./Images/PHP_2.png)
...
This iwll create a new blank file. paste the configuration below in the file.
